...
if [ ! -d "${TOOLSET_DIR}/apk/${ALPINE_VER}" ]; then
    ...
    pushd "${TOOLSET_DIR}"/apk
        ...
        apk_tools_static_path="${PIEMAN_DIR}/${TOOLSET_DIR}/apk/${ALPINE_VER}"
...
etalon_ownership="$(get_ownership "${PIEMAN_DIR}")"
if [ "$(get_ownership "${TOOLSET_DIR}")" != "${etalon_ownership}" ]; then
    info "correcting ownership for ${TOOLSET_DIR}"
    chown -R "${etalon_ownership}" "${TOOLSET_DIR}"
fi
